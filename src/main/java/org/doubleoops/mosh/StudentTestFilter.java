...
class StudentTestFilter implements ClassNameFilter {
	...
	public FilterResult apply(String className) {
		try {
			...
			if (MetaTestBase.class.isAssignableFrom(klazz)) {
				return FilterResult.excluded("Child of MetaTestBase");
			}
		...
		return FilterResult.included("Not a child of MetaTestBase");
	}
